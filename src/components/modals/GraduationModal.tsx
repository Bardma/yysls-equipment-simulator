...
const calcRate = (
  ...
  const totals = Calculator.calculateTotal(equips, currentClass, bowType, xinfa, setType, false, null, earlySeasonBonus);
...
const getTransmutationVariants = (equip: EquipItem, currentClass: string) => {
  ...
  const variants: Array<{ variant: EquipItem; subIndex: number; fromStat: string; toStat: string }> = [];
...
const findBestTransmutation = (
  ...
  if (!convertibleEquips.length) return { equippedItems: null, rate: 0, damage: 0, transmutations: [] as any[] };
  let bestResult = { equippedItems: null as EquippedItems | null, rate: 0, damage: 0, transmutations: [] as any[] };
  ...
  const traverse = (currentEquipped: EquippedItems, equipIndex: number, transmutations: any[]) => {
    if (equipIndex >= convertibleEquips.length) {
      if (minIntentRate !== null && minIntentRate !== undefined) {
        ...
        const intentRate = totals["实际会意率"] !== undefined ? totals["实际会意率"] : totals["会意率"] || 0;
        if (intentRate < minIntentRate) return;
      }
      const result = calculateBuildRate(currentEquipped, currentClass, bowType, setType, xinfa, earlySeasonBonus);
...
const getAllPossibleStats = (equippedItems: EquippedItems) => {
  ...
  const allStats = Object.keys(CommonData.MAX_VALUES).filter((stat) => !excludedStats.includes(stat));
  if (equippedItems.weapon1 && equippedItems.weapon2) {
    const weaponTypeStat1 = CommonData.WEAPON_TYPES.find((type) => type.id === equippedItems.weapon1?.weaponTypeId)?.stat;
    const weaponTypeStat2 = CommonData.WEAPON_TYPES.find((type) => type.id === equippedItems.weapon2?.weaponTypeId)?.stat;
...
const findBestStatsForSlotAsync = async (
  ...
  const effectiveSubCandidates = validSubStatCandidates.filter((stat) => !!CommonData.MAX_VALUES[stat]);
      ...
      for (let i = 0; i < n - 3; i++) {
        for (let j = i + 1; j < n - 2; j++) {
          for (let k = j + 1; k < n - 1; k++) {
            for (let m = k + 1; m < n; m++) {
              const combo = [subCandidates[i], subCandidates[j], subCandidates[k], subCandidates[m]];
              ...
              if (batchCount >= batchSize) {
                ...
                if (totalCombinations > 0) {
                  ...
                  progressCallback?.(`枚举组合中... ${checkedCombinations}/${totalCombinations} (${progress.toFixed(1)}%)`);
                }
...
export const GraduationModal = ({
  ...
  const cultivationSummary = useMemo(() => {
    ...
    (Object.keys(slotNameMap) as Array<keyof EquippedItems>).forEach((slotKey) => {
      ...
      if (mainStat && mainStat.type !== "生存类词条" && mainStat.type !== "生存向") {
        ...
        if (maxValue) {
          statSummary[mainStat.type] = (statSummary[mainStat.type] || 0) + mainStat.value / maxValue;
        }
      }
      equip.subStats.forEach((sub) => {
        if (sub.type !== "生存类词条" && sub.type !== "生存向") {
  ...
  const startBestBuild = async () => {
    ...
    db.forEach((equip) => {
      if (equip.slotId === "1") {
        ...
        const slotKey = { "3": "ring", "4": "pendant", "5": "head", "6": "chest", "7": "legs", "8": "hands" }[
          equip.slotId
        ] as keyof EquippedItems | undefined;
    ...
    let bestBuildB = { equippedItems: null as EquippedItems | null, rate: 0, damage: 0, transmutations: [] as any[] };

    function* combinationGenerator() {
      const stack: Array<{ equipped: EquippedItems; slotIndex: number }> = [
        { equipped: { weapon1: null, weapon2: null, head: null, chest: null, ring: null, pendant: null, legs: null, hands: null }, slotIndex: 0 },
      ];
      while (stack.length) {
    ...
    const processCombination = (currentEquipped: EquippedItems) => {
      ...
      const result = calculateBuildRate(currentEquipped, currentClass, bowType, setType, xinfaLoadout, earlySeasonBonus);
      if (result.rate > bestBuildA.rate) {
        bestBuildA = { equippedItems: addFullDingyinToEquips(currentEquipped), rate: result.rate, damage: result.damage };
      }
      if (result.rate >= bestBuildA.rate * 0.95) {
        const convertibleEquips = Object.entries(currentEquipped)
          ...
          .map(([slotKey, equip]) => ({ slotKey: slotKey as keyof EquippedItems, equip: equip as EquipItem }));
    ...
    await new Promise<void>((resolve) => {
      const processBatch = () => {
        ...
        const percent = totalCombinations ? Math.min(100, (checkedCount / totalCombinations) * 100) : 0;
        setBestBuildStatus({
          running: true,
  ...
  const startStatPriority = () => {
    ...
    allPossibleStats.forEach((statType) => {
      ...
      if (statType.includes("武学增效") && statType !== "全武学增效") {
        if (weapon1Sub.find((sub) => sub.type === statType) || weapon2Sub.find((sub) => sub.type === statType)) {
          statExist = true;
        }
      }
      ...
      if (!statExist) {
        ...
        const gainResult = Calculator.calculateGraduationRate(gainParams, skillDb, rotation, baselineValue, false);
      ...
      if (statExist || !statType.includes("武学增效") || statType === "全武学增效") {
        ...
        const lossResult = Calculator.calculateGraduationRate(lossParams, skillDb, rotation, baselineValue, false);
  ...
  const startCultivation = async () => {
    const slots = (Object.keys(slotNameMap) as Array<keyof EquippedItems>).filter((slotKey) => equippedItems[slotKey]);
    ...
    for (let idx = 0; idx < slots.length; idx++) {
      ...
      const result = Calculator.calculateGraduationRate(params, skillDb, rotation, baselineValue, false);
    ...
    if (bestImprovementEquip && bestImprovementEquip.slotKey !== "weapon1" && bestImprovementEquip.slotKey !== "weapon2") {
      ...
      if (weapon1 && weapon2) secondBestWeapon = weapon1.improvementSpace > weapon2.improvementSpace ? weapon1 : weapon2;
  ...
  const renderCompare = () => {
    if (compareCandidates.candidates.length === 0) {
      return <div className="text-center text-muted-foreground py-10">库中没有符合条件的同类装备可供对比</div>;
    }
    return (
      <div className="space-y-3">
        {compareCandidates.candidates.map((equip) => {
          ...
          if (freezeDingyin) testEquip.dingyinStat = compareCandidates.currentItem?.dingyinStat || null;
          const testLoadout = { ...equippedItems, [selectedSlotKey]: testEquip };
          const res = calcRate(testLoadout, currentClass, bowType, xinfaLoadout, setType, earlySeasonBonus);
          const newRate = parseFloat(res.graduationRate);
          const diff = newRate - currentRate;
          const diffColor = diff > 0.0001 ? "text-red-400" : diff < -0.0001 ? "text-green-400" : "text-muted-foreground";
          const diffSign = diff > 0.0001 ? "+" : "";
          return (
            <div key={equip.id} className="rounded-lg border border-border/60 bg-card p-3 space-y-2">
              <div className="flex items-center gap-3">
                <Image src={`/${equip.icon}`} alt={equip.name} width={48} height={48} className="rounded-md border" />
                <div className="flex-1">
                  <div className="font-medium">{equip.name}{assumeChengyin ? " (拟)" : ""}</div>
                  <div className="text-xs text-muted-foreground">毕业率: {newRate.toFixed(2)}%</div>
                </div>
  ...
  const renderConvertTab = () => {
    if (!convertTarget) {
      return (
        <div className="text-center text-muted-foreground py-10">
          ...
          <div className="mt-4">
            <Button onClick={() => handlePickEquip(selectedSlotKey, null)}>选择/录入装备进行分析</Button>
          </div>
        </div>
      );
    }
    ...
    const isElemental = (name: string) =>
      name.includes("鸣金") || name.includes("裂石") || name.includes("牵丝") || name.includes("破竹") || name.includes("无相");
    const normalize = (name: string) => (isWeapon && isElemental(name) ? name.replace(/鸣金|裂石|牵丝|破竹/g, "无相") : name);
    ...
    const analysisByArmory = Object.entries(armories).map(([armoryName, pool]) => {
      ...
      const existingStats = new Set(
        convertTarget.subStats.map((sub, idx) => (idx === selectedSubIndex ? "" : normalize(sub.type))).filter(Boolean)
      );
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between rounded-lg border border-border/60 bg-card p-3">
          <div className="text-sm text-muted-foreground">分析对象</div>
          <div className="font-medium">{convertTarget.name}</div>
          <Button size="sm" variant="secondary" onClick={() => handlePickEquip(selectedSlotKey, convertTarget)}>
            更换分析对象
          </Button>
        </div>
          ...
          {convertTarget.subStats.map((sub, idx) => (
            <button
              ...
              className={`px-3 py-1 rounded-md border text-sm ${
                idx === selectedSubIndex ? "border-primary text-primary" : "border-border/60 text-muted-foreground"
              }`}
        ...
        <div className="grid gap-3">
          {analysisByArmory.map((armory) => (
            <div key={armory.armoryName} className="rounded-lg border border-border/60 bg-card p-3">
              <div className="flex items-center justify-between">
                <div className="font-medium">{armory.armoryName}</div>
                <div className={`text-sm ${armory.expected > 0 ? "text-red-400" : armory.expected < 0 ? "text-green-400" : "text-muted-foreground"}`}>
                  期望 {armory.expected > 0 ? "+" : ""}{armory.expected.toFixed(2)}%
                </div>
              </div>
              <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                {armory.outcomes.map((outcome) => (
                  <div key={outcome.name} className="flex items-center justify-between">
                    ...
                    <span className={outcome.diff > 0 ? "text-red-400" : outcome.diff < 0 ? "text-green-400" : ""}>
                      {outcome.diff > 0 ? "+" : ""}{outcome.diff.toFixed(2)}%
                    </span>
                  </div>
                ))}
              </div>
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {(Object.keys(slotNameMap) as Array<keyof EquippedItems>).map((slotKey) => {
          ...
          return (
            <div key={slotKey} className="rounded-md border border-border/60 bg-card p-2 text-xs">
        ...
        <div className="grid grid-cols-[260px_1fr] gap-4">
          <div className="space-y-3">
            <div className="rounded-lg border border-border/60 bg-card p-3 text-center">
              <div className="text-xs text-muted-foreground">当前毕业率</div>
              <div className="text-xl font-semibold text-yellow-300">{accResult.graduationRate}</div>
              <div className="text-xs text-muted-foreground">excel表格显示：{excelResult.graduationRate}</div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">{slotNameMap[key]}</div>
                    </div>
                    {item ? <span className="text-xs text-muted-foreground">{getScore(item)}</span> : null}
                  </button>
                );
              })}
            </div>
          ...
          <div className="min-w-0">
            <Tabs defaultValue="compare" className="w-full">
              ...
              <TabsContent value="compare" className="pt-4 space-y-4">
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={assumeChengyin} onCheckedChange={(v) => setAssumeChengyin(Boolean(v))} />
                    假设满承音
                  </label>
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={freezeDingyin} onCheckedChange={(v) => setFreezeDingyin(Boolean(v))} />
                    冻结当前定音
                  </label>
                </div>
                {renderCompare()}
              ...
              <TabsContent value="best-build" className="pt-4 text-muted-foreground">
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="rounded-lg border border-border/60 bg-card p-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            size="sm"
                            onClick={() => onApplyBuild(bestBuildStatus.result.buildA.equippedItems)}
                          >
                            使用该方案
                          </Button>
                        </div>
                      ...
                      {bestBuildStatus.result.buildB && (
                        <div className="rounded-lg border border-border/60 bg-card p-3">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              size="sm"
                              onClick={() => onApplyBuild(bestBuildStatus.result.buildB.equippedItems)}
                            >
                              使用该方案
                            </Button>
                          </div>
              ...
              <TabsContent value="stat-priority" className="pt-4 text-muted-foreground">
                <div className="space-y-4">
                  <Button onClick={startStatPriority}>计算词条优先级</Button>
                  {statPriority ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <div className="font-medium text-sm">新增词条收益</div>
                      ...
                      <div className="space-y-2">
                        <div className="font-medium text-sm">扣除词条损失</div>
                    ...
                    <div className="text-sm text-muted-foreground">尚未计算。</div>
                  )}
                </div>
              </TabsContent>
              <TabsContent value="cultivation" className="pt-4 text-muted-foreground">
                <div className="space-y-4">
                  <div className="rounded-lg border border-border/60 bg-card p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {cultivationSummary.sortedStats.map(([stat, count]) => (
                        <div key={stat} className="rounded-md border border-border/60 bg-background/40 p-2 text-xs">
                          <div className="font-medium text-foreground">{stat}</div>
                    ...
                    <div className="rounded-md border border-border/60 bg-background/40 p-2 text-xs">
                      <div className="font-medium text-foreground">定音词条总结</div>
                      ...
                      {cultivationStatus.result.bestImprovementEquip && (
                        <div className="rounded-lg border border-border/60 bg-card p-3">
                          ...
                          <div className="text-sm text-muted-foreground">
                            ...
                            {cultivationStatus.result.bestImprovementEquip.improvementSpace.toFixed(2)}%
                          </div>
                          {cultivationStatus.result.bestImprovementEquip.bestMainStat && (
                            <div className="text-xs text-muted-foreground mt-2">
                              推荐主词条：{cultivationStatus.result.bestImprovementEquip.bestMainStat.stat}
                            </div>
                          )}
                          {cultivationStatus.result.bestImprovementEquip.bestSubStats?.length > 0 && (
                            <div className="text-xs text-muted-foreground mt-1">
                              推荐副词条：{cultivationStatus.result.bestImprovementEquip.bestSubStats
                      ...
                      {cultivationStatus.result.secondBestWeapon && (
                        <div className="rounded-lg border border-border/60 bg-card p-3">
                          ...
                          {cultivationStatus.result.secondBestWeapon.bestMainStat && (
                            <div className="text-xs text-muted-foreground mt-2">
                              推荐主词条：{cultivationStatus.result.secondBestWeapon.bestMainStat.stat}
                            </div>
                          )}
                          {cultivationStatus.result.secondBestWeapon.bestSubStats?.length > 0 && (
                            <div className="text-xs text-muted-foreground mt-1">
                              推荐副词条：{cultivationStatus.result.secondBestWeapon.bestSubStats
                                .map((stat: any) => stat.stat)
                                .join("、")}
                            </div>
                          )}
