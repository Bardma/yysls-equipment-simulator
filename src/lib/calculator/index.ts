...
export const Calculator = {
  ...
  _getCacheKey(
    ...
    const xinfaStr = Array.isArray(xinfaList)
      ? [...xinfaList].sort().join(",")
      : xinfaList;
    return `${currentClass}_${xinfaStr}_${setName}_${rotation.length}_${baseline}_${Object.keys(
      skillDb
    ).length}`;
  },
  ...
  calculateTotal(
    ...
    if (debug) {
      console.log(
        `- [弓类属性转化] 精准率: ${
          bowType === "precision" ? 4.7 : bowType === "crit" ? 5.2 : 2.6
        }`
      );
      console.groupEnd();
    }
  ...
  calculateGraduationRate(
    ...
    cachedSkillTimeline.forEach((action, idx) => {
      ...
      const ratioCrit =
        effCritRate + effIntentRate <= 1 ? effCritRate * effPrecision : effPrecision * (1 - effIntentRate);
      ...
      const outerPenBonus =
        (stats.outerPen + (skillData.exPen || 0) + dsBonus + shBonus + yiShuiBonus + threeQiongPenBonus + chuanHouPenBonus) /
        200;

      const specBonus = skillData.special === "鼠鼠" ? 0.24 : skillData.special === "回旋伞" ? 0.15 : 0;
      ...
      let critMult =
        1 + stats.critDmgBonus / 100 + (skillData.exCritDmg || 0) + dsBonus / 100;
      ...
      const dOutGlance = calcDmg(effMinOuter, skillData.outerRatio, outerPenBonus, 1 + effOuterDmgBonus, 1);
      const dOutCrit = calcDmg(effAvgOuter, skillData.outerRatio, outerPenBonus, 1 + effOuterDmgBonus, critMult);
      const dOutIntent = calcDmg(effMaxOuter, skillData.outerRatio, outerPenBonus, 1 + effOuterDmgBonus, intentMult);
      const dOutNormal = calcDmg(effAvgOuter, skillData.outerRatio, outerPenBonus, 1 + effOuterDmgBonus, 1);
      const outerExp = dOutGlance * ratioGlance + dOutCrit * ratioCrit + dOutIntent * ratioIntent + dOutNormal * ratioNormal;
      ...
      const fixedExp = dfNormal * (ratioGlance + ratioNormal) + dfCrit * ratioCrit + dfIntent * ratioIntent;
      ...
      const poZhuData = calcElementPart(
        ...
        stats.minPoZhu,
        stats.maxPoZhu,
        stats.poZhuPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus + poZhuElementPenBonus,
      ...
      const lieShiData = calcElementPart(
        ...
        stats.minLieShi,
        stats.maxLieShi,
        stats.lieShiPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus + lieShiElementPenBonus,
      ...
      if (debug) {
        ...
        if (skillData.exCrit) console.log(`  技能额外会心率: ${(skillData.exCrit * 100).toFixed(2)}%`);
        if (skillData.exIntent) console.log(`  技能额外会意率: ${(skillData.exIntent * 100).toFixed(2)}%`);
        console.log("  最终伤害占比:");
  ...
  addStatWithTrack(totalObj: Record<string, number>, statObj: { type: string; value: number }, trackFn?: (k: string, v: number) => void) {
