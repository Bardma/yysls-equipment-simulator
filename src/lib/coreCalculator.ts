/**
 * 核心计算器
 * 完全基于 http://spongem.com/yysls/js/core/calculator.js
 * 转换为 TypeScript 版本
 */

import { BASE_STATS, XINFA_DATA, SET_DATA } from './commonData';
import { ClassConfig, ROTATIONS } from './classConfig';
import type { SkillDatabase } from './skillData';

// 装备数据结构（用于计算器）
export interface CalculatorEquipment {
  slotId: string;
  slotName: string;
  name: string;
  isPurple: boolean;
  mainStat?: CalculatorStat;
  dingyinStat?: CalculatorStat;
  subStats: CalculatorStat[];
}
...
export class CoreCalculator {
  // 缓存机制：缓存技能循环相关的中间计算结果
  private _rotationCache: any = null;
  private _cacheKey: string | null = null;

  /**
   * 清除缓存（当流派、心法、套装等配置改变时调用）
   */
  clearCache(): void {
  ...
  private _getCacheKey(
    ...
    skillDb: Record<string, SkillData>,
    baseline: number
  ): string {
    const xinfaStr = Array.isArray(xinfaList) ? xinfaList.sort().join(',') : xinfaList;
    return `${currentClass}_${xinfaStr}_${setName}_${rotation.length}_${baseline}`;
  }

  /**
   * 添加属性并跟踪
   */
  private addStatWithTrack(
    totalObj: Record<string, number>,
    statObj: CalculatorStat | undefined,
   ...
   */
  calculateTotal(
    equippedItems: Record<string, CalculatorEquipment | null>,
    currentClass: string,
    bowType: string,
    xinfaList: string[] | string,
    setType: string,
    debug: boolean = false,
    statModifier: StatModifier | null = null,
    earlySeasonBonus: boolean = false
  ): Record<string, number> {
    let total = JSON.parse(JSON.stringify(BASE_STATS));
    let baseSanWei = BASE_STATS["敏"];
    ...
    Object.values(equippedItems).forEach((equip) => {
      ...
      let equipLog: { name: string; adds: Record<string, number> } = { name: equip.name, adds: {} };
      ...
      if (equip.slotId === '1') {
        ...
        let val = equip.isPurple ? 90 : 100;
        total["最小外功攻击"] = (total["最小外功攻击"] || 0) + val;
        track("最小外功攻击(白值)", val);
      } else if (equip.slotId === '4') {
        let val = equip.isPurple ? 135 : 150;
    ...
    if (xinfaList && Array.isArray(xinfaList)) {
      xinfaList.forEach((xinfaName) => {
        const stats = XINFA_DATA[xinfaName];
        if (stats) {
          for (const [k, v] of Object.entries(stats)) {
            total[k] = (total[k] || 0) + v;
            if (debug) {
              console.log(`- [${xinfaName}] ${k}:`, v);
            }
          }
        }
      });
    ...
    // 套装加成
    if (setType && SET_DATA[setType]) {
      const setStats = SET_DATA[setType];
      for (const [key, val] of Object.entries(setStats)) {
        total[key] = (total[key] || 0) + val;
        if (debug) {
          console.log(`- [${setType}] ${key}:`, val);
        }
      }
    }
    if (debug) {
    ...
    // 词条修改器：添加或删除一条词条（过滤掉"生存类词条"和"生存向"）
    if (statModifier && statModifier.type && statModifier.type !== '生存类词条' && statModifier.type !== '生存向') {
      ...
      if (statModifier.operation === 'add') {
        ...
        if (debug) {
          console.log(`- [${statModifier.type}] ${statModifier.value}:`, modifierValue);
        }
        ...
        if (statModifier.type === "劲" || statModifier.type === "敏" || statModifier.type === "势") {
    ...
    if (earlySeasonBonus) {
      ...
      if (debug) {
        console.log(`- [提前获得下半赛季属性] 劲: 14, 敏: 14, 势: 14，精准率: 1.4`);
      }
    }
    if (debug) {
      console.groupEnd();
    ...
    if (debug) {
      console.log(`- [属性转化] 最小外功攻击: ${extraJing * 0.22 + extraMin * 0.9}`);
    }
    total["最大外功攻击"] += extraJing * 1.36 + extraShi * 0.9;
    if (debug) {
      console.log(`- [属性转化] 最大外功攻击: ${extraJing * 1.36 + extraShi * 0.9}`);
    }
    total["会心率"] += extraMin * 0.076;
    if (debug) {
      console.log(`- [属性转化] 会心率: ${extraMin * 0.076}`);
    }
    total["会意率"] += extraShi * 0.038;
    if (debug) {
      console.log(`- [属性转化] 会意率: ${extraShi * 0.038}`);
    }
    if (currentClass === "鸣金虹") {
      ...
      if (debug) {
        console.log(`- [鸣金虹属性转化] 最大外功攻击: ${effectiveShi * 0.264}`);
      }
      if (debug) {
        console.log(`- [鸣金虹属性转化] 会意率: ${effectiveShi * 0.015}`);
      }
    }
    if (currentClass === "破竹鸢" || currentClass === "破竹尘" || currentClass === "破竹风" || currentClass === "牵丝玉" || currentClass === "牵丝霖" || currentClass === "裂石钧（双切）" || currentClass === "裂石钧（纯唐）") {
      const effectiveMin = Math.min(totalMin, 300);
      total["会心率"] += effectiveMin * 0.030;
      total["最小外功攻击"] += effectiveMin * 0.264;
      if (debug) {
        console.log(`- [会心职业属性转化] 最小外功攻击: ${effectiveMin * 0.264}`);
      }
      if (debug) {
        console.log(`- [会心职业属性转化] 会心率: ${effectiveMin * 0.030}`);
      }
    }
    if (currentClass === "裂石威") {
      const effectiveJing = Math.min(totalJing, 300);
      total["会心率"] += effectiveJing * 0.030;
      if (debug) {
        console.log(`- [裂石威属性转化] 会心率: ${effectiveJing * 0.030}`);
      }
    }
    if (currentClass === "鸣金影") {
      const effectiveJing = Math.min(totalJing, 300);
      ...
      if (debug) {
        console.log(`- [鸣金影属性转化] 最大外功攻击: ${effectiveJing * 0.264}`);
      }
      if (debug) {
        console.log(`- [鸣金影属性转化] 会意率: ${effectiveJing * 0.015}`);
      }
    }
    ...
    if (debug) {
      console.log(`- [弓类属性转化] 精准率: ${bowType === 'precision' ? 4.7 : bowType === 'crit' ? 5.2 : 2.6}`);
    }
    if (debug) {
      console.groupEnd();
    }
    ...
    let critOverflow = 0;
    if (finalCrit > 80) {
      critOverflow = (finalCrit - 80) * 1.85; // 溢出值（白值）
      finalCrit -= exCrit;
      if (finalCrit > 80) {
        finalCrit = 80;
      }
    } else {
      if (exCrit !== 0) finalCrit -= exCrit;
    }
    total["实际会心率"] = finalCrit;
    total["会心率溢出"] = critOverflow;
    if (debug) {
      console.log(`- [实际会心率] ${finalCrit}`);
    }
    if (debug) {
      console.log(`- [会心率溢出] ${critOverflow}`);
    }
    ...
    if (finalIntent > 40) {
      intentOverflow = (finalIntent - 40) * 1.85; // 溢出值（白值）
      finalIntent = 40;
    }
    total["实际会意率"] = finalIntent;
    total["会意率溢出"] = intentOverflow;
    if (debug) {
      console.log(`- [实际会意率] ${finalIntent}`);
    }
    if (debug) {
      console.log(`- [会意率溢出] ${intentOverflow}`);
    }
    ...
    let accOverflow = 0;
    if (finalAcc > 100) {
      accOverflow = (finalAcc - 100) * 1.85; // 溢出值（白值）
      finalAcc = 100;
    }
    total["实际精准率"] = finalAcc;
    total["精准率溢出"] = accOverflow;
    if (debug) {
      console.log(`- [实际精准率] ${finalAcc}`);
    }
    if (debug) {
      console.log(`- [精准率溢出] ${accOverflow}`);
    }
    //如果最终会心+最终会意+直接会心+直接会意>100，则会心溢出值还要加上(100-最终会心-最终会意-直接会心-直接会意)*1.85
    if (finalIntent + (finalCrit + exCrit > 80 ? 80 : finalCrit + exCrit) + total["直接会心率"] + total["直接会意率"] > 100) {
      const overflow = (finalIntent + (finalCrit + exCrit > 80 ? 80 : finalCrit + exCrit) + total["直接会心率"] + total["直接会意率"] - 100) * 1.85;
      total["会心率溢出"] += overflow;
      if (debug) {
        console.log(`- [会心率溢出] 额外溢出: ${overflow}`);
      }
    }
    if (debug) {
  ...
  calculateGraduationRate(
    ...
    // 获取当前流派
    const currentClass = String(params["当前流派"] || params["currentClass"] || "");
    const xinfaList = (params["心法"] as string[] | string) || [];
    const setName = String(params["套装"] || "");

    // 检查缓存：如果流派、心法、套装、循环、基准值相同，可以复用部分计算结果
    const cacheKey = this._getCacheKey(currentClass, xinfaList, setName, rotation, skillDb, baseline);
    const useCache = this._rotationCache && this._cacheKey === cacheKey;

    const getVal = (key: string) => parseFloat(String(params[key])) || 0;

    const maxAtks: Record<string, number> = {
      "破竹": getVal("最大破竹攻击"),
      "鸣金": getVal("最大鸣金攻击"),
      "裂石": getVal("最大裂石攻击"),
      "牵丝": getVal("最大牵丝攻击"),
    ...
    if (!useCache) {
      this._rotationCache = {
        ...
        setName: setName,
        xinfaList: xinfaList,
        checkXinfa: checkXinfa,
        hasSuoHen: hasSuoHen,
        hasDuanShi: hasDuanShi,
        hasYiShui: hasYiShui,
        bossDef: bossDef,
        weaponBonusMap: weaponBonusMap
      };
    ...
    // 复用缓存中的不变数据
    const cache = this._rotationCache;
    ...
    cachedSkillTimeline.forEach((action: RotationAction, idx: number) => {
      ...
      if (skillData.element && skillData.element !== "无" && skillData.element !== "N/A") {
        const eleKeyMap: Record<string, string> = { "破竹": "PoZhu", "鸣金": "MingJin", "裂石": "LieShi", "牵丝": "QianSi" };
        const suffix = eleKeyMap[skillData.element];
        if (suffix) {
          stats[`min${suffix}`] = (stats[`min${suffix}`] || 0) + stats.minWuXiang;
          stats[`max${suffix}`] = (stats[`max${suffix}`] || 0) + stats.maxWuXiang;
          stats[`${suffix.toLowerCase()}Pen`] = (stats[`${suffix.toLowerCase()}Pen`] || 0) + stats.wuXiangPen;
        }
      }

      // 2. 概率
      let effCritRate = (stats.critRate / 100) + (skillData.exCrit || 0) + (cachedSetName === "浣花" ? 5 : 0);
      ...
      let ratioIntent = effIntentRate;
      let ratioCrit = (effCritRate + effIntentRate <= 1) ? effCritRate * effPrecision : effPrecision * (1 - effIntentRate);
      let ratioNormal = Math.max(0, 1 - ratioGlance - ratioCrit - ratioIntent);

      // 3. 全局增伤
      let weaponBonus = 0;
      if ((skillData.type === "武器" || skillData.type === "心法") && skillData.weaponType !== "N/A") {
      ...
      let finalGlobalMult = 1 + (action.generalBonus || 0) + (stats.bossDmgBonus / 100) + weaponBonus;
      ...
      let effAvgOuter = (effMinOuter + effMaxOuter) / 2;

      const isDuanShiSkill = !!skillData.modifiers?.["断石"];
      let dsBonus = (isDuanShiSkill && cachedHasDuanShi) ? 25 : 0;
      let shBonus = cachedHasSuoHen ? 10 : 0;
      let yiShuiBonus = (cachedHasYiShui && action.yishui) ? action.yishui : 0;
      let threeQiongPenBonus = (skillData.modifiers?.["三穷"] === 2 && cachedCheckXinfa("三穷致知")) ? 20 : 0;
      let chuanHouPenBonus = (skillData.modifiers?.["穿喉"] && skillData.modifiers["穿喉"] > 0 && cachedCheckXinfa("穿喉决")) ? skillData.modifiers["穿喉"] : 0;

      let outerPenBonus = (stats.outerPen + (skillData.exPen || 0) + dsBonus + shBonus + yiShuiBonus + threeQiongPenBonus + chuanHouPenBonus) / 200;

      let specBonus = (skillData.special === "鼠鼠" ? 0.24 : (skillData.special === "回旋伞" ? 0.15 : 0));
      ...
      if (skillData.modifiers?.["穿喉"] && skillData.modifiers["穿喉"] > 0 && cachedCheckXinfa("穿喉决")) {
        critMult += skillData.modifiers["穿喉"] / 100;
      }

      let dOutGlance = calcDmg(effMinOuter, skillData.outerRatio || 0, outerPenBonus, 1 + effOuterDmgBonus, 1);
      let dOutCrit = calcDmg(effAvgOuter, skillData.outerRatio || 0, outerPenBonus, 1 + effOuterDmgBonus, critMult);
      let dOutIntent = calcDmg(effMaxOuter, skillData.outerRatio || 0, outerPenBonus, 1 + effOuterDmgBonus, intentMult);
      let dOutNormal = calcDmg(effAvgOuter, skillData.outerRatio || 0, outerPenBonus, 1 + effOuterDmgBonus, 1);
      let outerExp = (dOutGlance * ratioGlance) + (dOutCrit * ratioCrit) + (dOutIntent * ratioIntent) + (dOutNormal * ratioNormal);

      // 5. 固伤板块
      let effFixed = skillData.fixed || 0;
      if (skillData.type === "武器") effFixed *= (1 + stats.fixedDmgBonus);
      let fixedPenBonus = outerPenBonus;
      let fixedDmgMultiplier = 1 + effOuterDmgBonus;

      let dfNormal = calcDmg(effFixed, 1.0, fixedPenBonus, fixedDmgMultiplier, 1);
      let dfCrit = calcDmg(effFixed, 1.0, fixedPenBonus, fixedDmgMultiplier, critMult);
      let dfIntent = calcDmg(effFixed, 1.0, fixedPenBonus, fixedDmgMultiplier, intentMult);
      let fixedExp = (dfNormal * (ratioGlance + ratioNormal)) + (dfCrit * ratioCrit) + (dfIntent * ratioIntent);

      // 6. 属性板块
      function calcElementPart(eleName: string, minEle: number, maxEle: number, elePen: number, eleDmgBonus: number) {
        let eleSetMult = (cachedSetName === "撼天") ? 1.05 : 1.0;
        let extraEleAtk = (skillData.type === "武器" && skillData.element === eleName) ? 150.7 * (1 + stats.fixedDmgBonus) : 0;
        ...
        let effAvgEle = (effMinEle + effMaxEle) / 2;
        let elePenBonus = (elePen) / 200;
        let usedRatio = (skillData.element === eleName) ? (skillData.eleRatio || 0) : (skillData.outerRatio || 0);
        let effEleDmgBonus = eleDmgBonus / 100;

        let dGlance = calcDmg(effMinEle, usedRatio, elePenBonus, 1 + effEleDmgBonus, 1);
        let dNormal = calcDmg(effAvgEle, usedRatio, elePenBonus, 1 + effEleDmgBonus, 1);
        let dCrit = calcDmg(effAvgEle, usedRatio, elePenBonus, 1 + effEleDmgBonus, critMult);
        let dIntent = calcDmg(effMaxEle, usedRatio, elePenBonus, 1 + effEleDmgBonus, intentMult);
        return {
          exp: (dGlance * ratioGlance) + (dNormal * ratioNormal) + (dCrit * ratioCrit) + (dIntent * ratioIntent),
          min: effMinEle,
          max: effMaxEle,
          avg: effAvgEle,
          dGlance: dGlance,
          ...
          extraEleAtk: extraEleAtk,
          eleSetMult: eleSetMult
        };
      }
      ...
      if (skillData.modifiers?.["三穷"] === 1 || (skillData.modifiers?.["三穷"] === 2 && cachedCheckXinfa("三穷致知"))) {
        threeQiongBonus = 20;
      }
      if ((skillData.special === "撼天" || skillData.special === "鼠鼠") && cachedSetName === "撼天") {
        hanTianPenBonus += 4;
      }
      if (cachedCurrentClass === "破竹鸢" && !action.name.includes("无返豆")) {
      ...
      let poZhuData = calcElementPart("破竹", stats.minPoZhu, stats.maxPoZhu, stats.poZhuPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus + poZhuElementPenBonus, stats.poZhuDmgBonus + poZhuElementDmgBonus);
      let mingJinData = calcElementPart("鸣金", stats.minMingJin, stats.maxMingJin, stats.mingJinPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus + mingJinPenBonus, stats.mingJinDmgBonus);
      let lieShiData = calcElementPart("裂石", stats.minLieShi, stats.maxLieShi, stats.lieShiPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus + lieShiElementPenBonus, stats.lieShiDmgBonus);
      let qianSiData = calcElementPart("牵丝", stats.minQianSi, stats.maxQianSi, stats.qianSiPen + threeQiongBonus + hanTianPenBonus + allElementPenBonus, stats.qianSiDmgBonus);

      let poZhuExp = poZhuData.exp;
      let mingJinExp = mingJinData.exp;
      let lieShiExp = lieShiData.exp;
      let qianSiExp = qianSiData.exp;

      let totalPartExp = outerExp + fixedExp + poZhuExp + mingJinExp + lieShiExp + qianSiExp;
      let finalSkillDmg = totalPartExp * finalGlobalMult * 1;

      if (action.isDingyin) finalSkillDmg *= (1 + (stats.specificSkillBonus / 100));
      let totalDmgForAction = finalSkillDmg * action.count;
    ...
    if (currentClass === "破竹尘") {
      settlementBonusRate = 0.1;
    }
    if (currentClass === "破竹风") {
      settlementBonusRate = 0.3;
    }
    ...
    // 使用传入的baseline，如果没有则使用默认值
    const baselineValue = adjustedBaseline || 4244078.34;
...
// 导出单例
export const coreCalculator = new CoreCalculator();
